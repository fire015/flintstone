...
class Config
{
	...
	{
		if (isset($config['dir'])) $this->setDir($config['dir']);
		if (isset($config['ext'])) $this->setExt($config['ext']);
		if (isset($config['gzip'])) $this->setGzip($config['gzip']);
		if (isset($config['cache'])) $this->setCache($config['cache']);
		if (isset($config['formatter'])) $this->setFormatter($config['formatter']);
		if (isset($config['swap_memory_limit'])) $this->setSwapMemoryLimit($config['swap_memory_limit']);
	}
	...
	{
		if (!is_bool($cache) && !($cache instanceof Cache))
		{
			throw new Exception('Cache must be a boolean or an instance of Doctrine\Common\Cache\Cache');
		}
	...
	{
		if (is_null($this->config['formatter'])) {
